...
function handle(req, res) {
  const cookies = new Cookies(req, res, { secure: req.headers['x-forwarded-proto'] === 'https' });
  switch (req.method) {
    case 'GET':
      ...
      for (let i = 1; i <= 3; i++) {
        ...
        if (savedEvent) {
          try {
            ...
            parsed.formattedDate = `${d.getFullYear()}/${d.getMonth() + 1}/${d.getDate()} ${String(d.getHours()).padStart(2, '0')}:${String(d.getMinutes()).padStart(2, '0')}`;
  ...
  switch (req.method) {
    case 'POST':
      ...
      req.on('data', (chunk) => {
        ...
        const pinnedSlot = cookies.get('pinnedSlot');

        if (pinnedSlot === slot) {
          cookies.set('pinnedSlot', '', {
            maxAge: 0,
            path: '/',
            httpOnly: true,
            sameSite: 'strict',
